...
const canWeDeliver = zipCode => {
    if (deliveryAreaZipCodes.includes(zipCode)) {
        console.log(`Yes we deliver there!`)
    } else {
        console.log(`We dont deliver there.`)
    }
}
...
const canWeDeliverTwo = arr1 => {
    for (let i = 0; i < deliveryAreaZipCodes.length; i++) {
        if (arr1[i] === deliveryAreaZipCodes) {
            console.log(`We deliver!`);
        } else {
            console.log(`We dont.`)
        }
    }
...
let deals2 = deals.findIndex((obj => obj.title == 1));
deals[deals2].title = '15% off';
